...
int hypfs_read_dyndir_entry(const struct hypfs_entry *template,
                            ...
                            bool is_last, XEN_GUEST_HANDLE_PARAM(void) *uaddr)
{
...
static struct hypfs_entry *hypfs_fill_dyndir_entry(
    struct hypfs_dyndir *dyndata, const struct hypfs_entry *template,
    const char *name)
{
...
struct hypfs_entry *hypfs_gen_dyndir_entry(
    ...
    void *data)
{
    struct hypfs_dyndir *dyndata;

    dyndata = hypfs_get_dyndata();
    dyndata->data = data;

    return hypfs_fill_dyndir_entry(dyndata, template, name);
}

/*
 * Fill dyndata with a dynamically generated entry based on a template
 * and a name. Put read-only content.
 * Needs to be kept in sync with hypfs_read_dyndir_entry() regarding the
 * name generated.
 */
struct hypfs_entry *hypfs_gen_dyndir_ro_entry(
    const struct hypfs_entry *template, const char *name,
    const void *content)
{
    struct hypfs_dyndir *dyndata;
    ...
    const struct hypfs_entry_dir *template, unsigned int id, void *data)
{
    ...
    dyndata->id.id = id;
    dyndata->data = data;
    snprintf(dyndata->id.name, sizeof(dyndata->id.name),
             template->e.name, id);
    dyndata->dir = *template;
    dyndata->dir.e.name = dyndata->id.name;
    dyndata->dir.e.funcs = &dyndata->funcs;
