 ...
 */
static uint32_t pack_scmi_header(scmi_msg_header_t *hdr)
{
 ...
 */
static void unpack_scmi_header(uint32_t msg_hdr, scmi_msg_header_t *hdr)
{
...
static int channel_is_free(struct scmi_channel *chan_info)
{
                            ...
                            scmi_msg_header_t *hdr, void *data, int len)
{
    ...
    chan_info->shmem->channel_status = 0;
    /* Writing 0 right now, but SCMI_SHMEM_FLAG_INTR_ENABLED can be set */
    chan_info->shmem->flags = 0;
...
static int get_shmem_regions(struct list_head *head, uint64_t hyp_addr)
{
    struct dt_device_node *node;
    int ret;
    struct dt_channel_addr *lchan;
    uint64_t laddr, lsize;
...
static __init bool scmi_probe(struct dt_device_node *scmi_node)
{
    ...
    list_for_each_entry(entry, &addr_list, list)
    {
        ...
        agent_channel = smc_create_channel(i, func_id, entry->addr);
                           ...
                           struct xen_arch_domainconfig *config)
{
    ...
    printk(XENLOG_DEBUG "scmi: domain_id = %d\n", d->domain_id);
    ...
    {
        ret = mem_permit_access(d, channel->paddr, SCMI_SHMEM_MAPPED_SIZE);
        if ( IS_ERR_VALUE(ret) )
            goto error;

        ret = dt_update_domain_range(channel->paddr, SCMI_SHMEM_MAPPED_SIZE);
        if ( IS_ERR_VALUE(ret) )
        {
            int rc = mem_deny_access(d, channel->paddr, SCMI_SHMEM_MAPPED_SIZE);
            if ( rc )
...
static int scmi_add_dt_device(struct domain *d, struct dt_device_node *dev)
{
    ...
    printk(XENLOG_DEBUG "scmi: dt_node = %s\n", dt_node_full_name(dev));
