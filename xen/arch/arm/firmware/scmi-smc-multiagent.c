                            ...
                            scmi_msg_header_t *hdr, void *data, int len)
{
    ...
    if ( resp.a0 == (unsigned long)ARM_SMCCC_INVALID_PARAMETER )
        return -EINVAL;
...
static void free_channel_list(void)
{
    ...
    /*
     * Called only on the __init error path, before any runtime users exist,
     * so no other thread is iterating channel_list. Keep the lock for the
     * whole drain for clarity and to avoid misleading readers about possible
     * concurrent access.
     */
    spin_lock(&scmi_data.channel_list_lock);
