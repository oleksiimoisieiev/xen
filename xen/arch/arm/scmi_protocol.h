...
#define SCMI_SHMEM_CHAN_STAT_CHANNEL_FREE	BIT(0, UL)
	__le32 reserved1[2];
	__le32 flags;
#define SCMI_SHMEM_FLAG_INTR_ENABLED	BIT(0, UL)
	__le32 length;
	__le32 msg_header;
	u8 msg_payload[0];
};
...
struct scmi_msg_resp_perf_attributes {
	__le16 num_domains;
	__le16 flags;
#define POWER_SCALE_IN_MILLIWATT(x)	((x) & BIT(0, UL))
...
struct scmi_msg_resp_perf_domain_attributes {
	__le32 flags;
#define SUPPORTS_SET_LIMITS(x)		((x) & BIT(31, UL))
#define SUPPORTS_SET_PERF_LVL(x)	((x) & BIT(30, UL))
#define SUPPORTS_PERF_LIMIT_NOTIFY(x)	((x) & BIT(29, UL))
#define SUPPORTS_PERF_LEVEL_NOTIFY(x)	((x) & BIT(28, UL))
