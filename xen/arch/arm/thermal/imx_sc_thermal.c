        ...
        enum thermal_trend *trend)
{
    ...
    if ( temp > sc_sensor->last_temp )
        *trend = THERMAL_TREND_RAISING;
    else if ( temp < sc_sensor->last_temp )
    ...
    if ( (*trend == THERMAL_TREND_DROPPING) && (trip_temp == temp) )
        *trend = THERMAL_TREND_DROP_FULL;
out:
    sc_sensor->last_temp = temp;
    return 0;
}
...
static void imx_sc_thermal_work(void *data)
{
    ...
    if ( ABS(temp - sensor->trips[PASSIVE_ID].temp) <
            sensor->trips[PASSIVE_ID].hyst )
    {
        trip = PASSIVE_ID;
        notify = true;
    }
    else if ( ABS(temp - sensor->trips[CRITICAL_ID].temp) <
